...
public class mojaTrzeciaAplikacja {
    public static void main(String[] args)
    {
        ...
        while (true) {
            System.out.print("Wpisz liczbę dni (lub wartość ujemną, aby zakończyć): ");
            int liczbaDni = scanner.nextInt();

            if (liczbaDni < 0) {
                System.out.println("Zakończenie programu przeliczania dni.");
                break;
            }

            int liczbaTygodni = liczbaDni / 7; // Obliczanie ilości tygodni
            int resztaDni = liczbaDni % 7; // Obliczanie pozostałych dni

            System.out.println(liczbaDni + " dni to " + liczbaTygodni + " tygodni i " + resztaDni + " dni.");
        }

        // Zadanie 2: Przeliczanie temperatury
        while (true) {
            System.out.print("Wpisz temperaturę w stopniach Fahrenheita (lub -1, aby zakończyć): ");
            double tempFahrenheit = scanner.nextDouble();

            if (tempFahrenheit == -1) {
                System.out.println("Zakończenie programu przeliczania temperatury.");
                break;
            }

            double tempCelsjusz = (tempFahrenheit - 32) / 1.8; // Konwersja na Celsjusza
            double tempKelwin = tempCelsjusz + 273.16; // Konwersja na Kelwina

            System.out.printf("Fahrenheit: %.2f, Celsjusz: %.2f, Kelwin: %.2f\n", tempFahrenheit, tempCelsjusz, tempKelwin);
        }
